...
void dijkstra::inputData() {
	fp.open("11аж - Dijkstra.txt");
	...
	while (!minHeap.isEmpty()) {
		...
		if (!flag[u.vertex]) {
			for (list* search = graph->getHead(u.vertex); search; search = search->getNext()) {
				if (!flag[search->getVertex()] && d[search->getVertex()] > d[u.vertex] + search->getWeight()) {
					p[search->getVertex()] = u.vertex;
					d[search->getVertex()] = d[u.vertex] + search->getWeight();
